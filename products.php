...
<h2>Catalogo de Cassettes de Edicion Limitada</h2>
...
<script>
        ...
        if (!anyVisible) {
            if (!msg) {
                ...
                Object.assign(msg.style, {
                    ...
                    color: '#ff2a6d', fontFamily: 'monospace'
                });
                msg.innerText = 'No se encontraron peliculas con ese nombre.';
    ...
    document.querySelectorAll('.paypal-button-container').forEach(container => {
        ...
        paypal.Buttons({
            ...
            //Crear orden en el servidor Node
            createOrder: async () => {
                ...
                try {
                    const response = await fetch(`${NODE_API}/api/create-paypal-order`, {
                        ...
                        body:    JSON.stringify({ amount: price, product: productName })
                    });

                    if (!response.ok) {
                        const err = await response.text();
                        throw new Error(`Servidor respondio ${response.status}: ${err}`);
                    }

                    const order = await response.json();

                    if (!order.id) {
                        throw new Error('El servidor no devolvio un ID de orden válido.');
                    }
            ...
            //Capturar el pago despues de que el usuario aprobo el pago en la ventana de PayPal
            onApprove: async (data) => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(`Captura fallo con HTTP ${response.status}`);
                    }

                    const details = await response.json();
                    const nombre  = details.payer?.name?.given_name || 'cliente';
                    statusDiv.innerHTML = `¡Pago completado, ${nombre}! Cinta en camino`;
            ...
            //El usuario cancela en la ventana de PayPal
            onCancel: () => {
                statusDiv.innerHTML = 'Pago cancelado.';
            },
